...
function ddw_gftb_admin_bar_menu() {
	...
	if ( ( defined( 'GFTB_DE_DISPLAY' ) && GFTB_DE_DISPLAY )
		&& ( get_locale() == 'de_DE' || get_locale() == 'de_AT' || get_locale() == 'de_CH' || get_locale() == 'de_LU' )
	) {

		$gfgroup_menu_items[ 'languages-de' ] = array(
			'parent' => $gfgroup_check_item,
			'title'  => __( 'German language files', 'gravity-forms-toolbar' ),
			'href'   => 'http://deckerweb.de/sprachdateien/gravityforms-und-addons/',
	...
	if ( ( defined( 'GFTB_NL_DISPLAY' ) && GFTB_NL_DISPLAY )
		&& ( ( get_locale() == 'nl' || get_locale() == 'nl_NL' ) && ! class_exists( 'GravityFormsNL' ) )
	) {

		$gfgroup_menu_items[ 'languages-nl' ] = array(
			'parent' => $gfgroup_check_item,
			'title'  => __( 'Dutch language plugin', 'gravity-forms-toolbar' ),
			'href'   => 'http://wordpress.org/plugins/gravityforms-nl/',
	...
	if ( class_exists( 'RGForms' ) ) {
		...
		if ( current_user_can( 'gravityforms_view_settings' ) || current_user_can( 'gform_full_access' ) ) {
			...
			$gftb_api_string = ddw_gftb_l10n_global_string_output( 'Gravity Forms API Settings' );
			$gftb_api_string = str_replace( ' ', '+', $gftb_api_string );
